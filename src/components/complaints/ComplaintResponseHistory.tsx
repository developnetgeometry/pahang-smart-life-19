...
export default function ComplaintResponseHistory({
  complaintId,
  refreshKey,
}: ComplaintResponseHistoryProps) {
  ...
  const fetchResponses = async () => {
    try {
      ...
      const responderIds = [
        ...new Set(
          responsesData?.map((r) => r.responder_id).filter(Boolean) || []
        ),
      ];
      ...
      // Combine the data
      const typedResponses: ComplaintResponse[] = (responsesData || []).map(
        (item) => {
          const responderProfile = profilesData.find(
            (p) => p.id === item.responder_id
          );
          return {
            id: item.id,
                        ...
                        <div className="flex-1">
                          <p className="text-xs font-medium text-yellow-700 dark:text-yellow-300 mb-1">
                            {language === "en"
                              ...
                              : "Komen Dalaman (Kakitangan Sahaja)"}
                          </p>
