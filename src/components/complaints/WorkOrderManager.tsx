...
import { Plus, Wrench, Calendar, Clock, User, AlertTriangle, ArrowUpRight } from 'lucide-react';

interface WorkOrder {
  id: string;
  title: string;
  description: string;
  work_order_type: 'maintenance' | 'repair' | 'inspection' | 'emergency' | 'general';
...
export default function WorkOrderManager({ 
  ...
  const [maintenanceStaff, setMaintenanceStaff] = useState<Array<{id: string, name: string}>>([]);
  ...
  const fetchWorkOrders = async () => {
    ...
    try {
      ...
      toast({
        title: language === 'en' ? 'Error loading work orders' : 'Ralat memuatkan arahan kerja',
        variant: 'destructive'
      });
    } finally {
      setLoading(false);
  ...
  const handleCreateWorkOrder = async () => {
    ...
    try {
      ...
      toast({
        title: language === 'en' ? 'Work order created successfully' : 'Arahan kerja berjaya dicipta',
      });
      ...
      toast({
        title: language === 'en' ? 'Error creating work order' : 'Ralat mencipta arahan kerja',
        variant: 'destructive'
      });
    } finally {
      setSubmitting(false);
        ...
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                {complaintId 
                  ? (language === 'en' ? 'Create a work order to resolve this complaint' : 'Cipta arahan kerja untuk menyelesaikan aduan ini')
                  : (language === 'en' ? 'Create a new work order for maintenance tasks' : 'Cipta arahan kerja baru untuk tugas penyelenggaraan')
                }
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <Label htmlFor="title">
                  {language === 'en' ? 'Work Order Title' : 'Tajuk Arahan Kerja'}
                </Label>
                <Input 
                  id="title" 
                  value={formData.title}
                  onChange={(e) => setFormData(prev => ({ ...prev, title: e.target.value }))}
                  placeholder={language === 'en' ? 'Brief description of the work needed' : 'Penerangan ringkas kerja yang diperlukan'} 
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={formData.work_order_type} onValueChange={(value: any) => setFormData(prev => ({ ...prev, work_order_type: value }))}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      ...
                      <SelectItem value="repair">{language === 'en' ? 'Repair' : 'Pembaikan'}</SelectItem>
                      <SelectItem value="inspection">{language === 'en' ? 'Inspection' : 'Pemeriksaan'}</SelectItem>
                      <SelectItem value="emergency">{language === 'en' ? 'Emergency' : 'Kecemasan'}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.priority} onValueChange={(value: any) => setFormData(prev => ({ ...prev, priority: value }))}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="urgent">{language === 'en' ? 'Urgent' : 'Segera'}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  placeholder={language === 'en' ? 'Where the work needs to be done' : 'Di mana kerja perlu dilakukan'} 
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea 
                  ...
                  placeholder={language === 'en' ? 'Detailed description of the work required...' : 'Penerangan terperinci kerja yang diperlukan...'} 
                  rows={4}
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="estimated_cost">
                    {language === 'en' ? 'Est. Cost (MYR)' : 'Anggaran Kos (MYR)'}
                  </Label>
                  <Input 
                    id="estimated_cost" 
                    type="number"
                ...
                <div className="space-y-2">
                  <Label htmlFor="scheduled_date">
                    {language === 'en' ? 'Scheduled Date' : 'Tarikh Dijadualkan'}
                  </Label>
                  <Input 
                    id="scheduled_date" 
                    type="date"
              ...
              <div className="space-y-2">
                ...
                <Select value={formData.assigned_to} onValueChange={(value) => setFormData(prev => ({ ...prev, assigned_to: value }))}>
                  ...
                  <SelectContent>
                    <SelectItem value="">{language === 'en' ? 'Unassigned' : 'Tidak ditugaskan'}</SelectItem>
              ...
              <div className="space-y-2">
                ...
                <Textarea 
                  ...
                  placeholder={language === 'en' ? 'Any additional instructions or requirements...' : 'Sebarang arahan atau keperluan tambahan...'} 
                  rows={3}
                />
              </div>
              ...
              <div className="flex justify-end space-x-2 pt-4">
                ...
                >
                  {submitting && <Clock className="w-4 h-4 mr-2 animate-spin" />}
                  {language === 'en' ? 'Create Work Order' : 'Cipta Arahan Kerja'}
                </Button>
              </div>
            </div>
          ...
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <h3 className="text-lg font-semibold mb-2">
                {language === 'en' ? 'No work orders yet' : 'Tiada arahan kerja lagi'}
              </h3>
              ...
              <CardContent className="p-6">
                <div className="flex items-start justify-between">
                  <div className="flex-1 space-y-3">
                    ...
                    {(workOrder.estimated_hours || workOrder.estimated_cost) && (
                      <div className="flex items-center gap-4 text-sm">
                        {workOrder.estimated_hours && (
                          <span className="text-muted-foreground">
                            {language === 'en' ? 'Est: ' : 'Anggaran: '}{workOrder.estimated_hours}h
                          </span>
                        )}
