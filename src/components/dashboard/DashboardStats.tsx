...
export function DashboardStats() {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        if (
          ...
          const [complaintsRes, announcementsRes, profilesRes] =
            await Promise.all([
              supabase
                .from("complaints")
    ...
    ? [
        {
          ...
          icon: AlertTriangle,
          description:
            language === "en" ? "Awaiting response" : "Menunggu respons",
        ...
        },
        {
          ...
          icon: Users,
          description:
            language === "en" ? "Total published" : "Jumlah diterbitkan",
          trend:
            language === "en" ? "Community updates" : "Kemas kini komuniti",
          color: "bg-primary",
        },
      ]
    : [];
  ...
  const professionalStats = stats
    ? [
        ...
        {
          title:
            language === "en" ? "Total Announcements" : "Jumlah Pengumuman",
          value: stats.announcements.toString(),
          icon: Activity,
          description:
            language === "en" ? "Published notices" : "Notis diterbitkan",
          trend:
            language === "en" ? "Community updates" : "Kemas kini komuniti",
          color: "bg-gradient-community",
        },
  ...
  const showProfessionalStats =
    hasRole("state_admin") ||
    hasRole("community_admin") ||
    hasRole("security_officer");

  const displayStats = showProfessionalStats
    ? professionalStats
    : residentStats;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {displayStats.map((stat, index) => (
        <Card
          key={index}
          className="relative overflow-hidden hover:shadow-elegant transition-spring group"
        >
          ...
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <div
              className={`p-2 rounded-lg ${stat.color} group-hover:shadow-glow transition-spring`}
            >
