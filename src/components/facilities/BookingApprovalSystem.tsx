...
import { Calendar, Clock, User, MapPin, CheckCircle, XCircle, MessageSquare } from 'lucide-react';
    ...
    try {
      ...
      await supabase
        ...
        .insert({
          ...
          message: `Your booking for ${selectedBooking.facilities.name} on ${new Date(selectedBooking.booking_date).toLocaleDateString()} has been ${actionType === 'approve' ? 'approved' : 'rejected'}.${notes ? ` Note: ${notes}` : ''}`,
      ...
      {bookings.length === 0 ? (
        <Card>
          <CardContent className="text-center py-8">
            ...
            <p className="text-muted-foreground">No pending booking requests to review.</p>
          </CardContent>
        </Card>
      ) : (
            ...
            <Card key={booking.id} className="border-l-4 border-l-orange-500">
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div>
                    <CardTitle className="text-lg">{booking.facilities.name}</CardTitle>
          ...
          {selectedBooking && (
            <div className="space-y-4">
              <div className="p-3 bg-muted rounded-lg">
                <h4 className="font-medium">{selectedBooking.facilities.name}</h4>
                <p className="text-sm text-muted-foreground">
                  {selectedBooking.profiles.full_name} • {new Date(selectedBooking.booking_date).toLocaleDateString()} • {selectedBooking.start_time} - {selectedBooking.end_time}
                </p>
              </div>
