...
export default function Services() {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      ...
      try {
        ...
        const advertiserIds = [
          ...new Set((data || []).map((item) => item.advertiser_id)),
        ];
        ...
        const transformedServices: ServiceAdvertisement[] = (data || []).map(
          (item) => ({
            id: item.id,
            title: item.title,
  ...
  const handleServiceClick = async (serviceId: string) => {
    ...
    await supabase
      .from("advertisements")
      .update({
        click_count: services.find((s) => s.id === serviceId)?.click_count + 1,
      })
      .eq("id", serviceId);
    ...
    try {
      ...
      if (!user) {
        toast({
          ...
          description:
            language === "en"
              ? "Please login to create demo data"
              : "Sila log masuk untuk mencipta data demo",
          variant: "destructive",
        });
        return;
      }
      ...
      await supabase.from("advertisements").insert({
        ...
        description:
          "Expert plumbing services for residential and commercial properties. Quick response time and affordable rates.",
      ...
      toast({
        ...
        description:
          language === "en"
            ? "Demo service advertisement created successfully."
            : "Iklan perkhidmatan demo berjaya dicipta.",
      });
      ...
      toast({
        ...
        description:
          language === "en"
            ? "Failed to create demo advertisement"
            : "Gagal mencipta iklan demo",
        variant: "destructive",
      });
    } finally {
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Avatar className="h-6 w-6">
                          <AvatarImage
                            src={service.advertiser_profile?.avatar_url}
                          />
                          <AvatarFallback>
                            {service.advertiser_profile?.full_name?.charAt(0) ||
                              "S"}
                          </AvatarFallback>
                        </Avatar>
                        <span className="text-xs text-muted-foreground">
                          {service.advertiser_profile?.full_name ||
                            "Service Provider"}
                        </span>
                      </div>
                      {service.price && (
