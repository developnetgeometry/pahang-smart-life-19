...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (roleError || !userRoles) {
      return new Response(JSON.stringify({ error: 'Failed to check user permissions' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (req.method === 'GET') {
      ...
      if (!hostUserId) {
        return new Response(JSON.stringify({ error: 'host_user_id parameter required' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
      ...
      if (accountsError) {
        ...
        return new Response(JSON.stringify({ error: 'Failed to fetch household accounts' }), {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
    ...
    if (req.method === 'POST') {
      ...
      const { data: existingUser, error: checkError } = await supabaseAdmin.auth.admin.getUserByEmail(body.tenant_email);
      ...
      if (hostError || !hostProfile) {
        return new Response(JSON.stringify({ error: 'Failed to fetch host user details' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
      ...
      if (profileError) {
        ...
        return new Response(JSON.stringify({ error: 'Failed to create tenant profile' }), {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
      ...
      if (roleError) {
        ...
        return new Response(JSON.stringify({ error: 'Failed to assign role to tenant' }), {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
      ...
      if (householdError) {
        ...
        await supabaseAdmin.auth.admin.deleteUser(newUser.user.id);
        await supabaseAdmin.from('profiles').delete().eq('id', newUser.user.id);
        await supabaseAdmin.from('enhanced_user_roles').delete().eq('user_id', newUser.user.id);
        return new Response(JSON.stringify({ error: 'Failed to create household account link' }), {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
      ...
      const { error: resetError } = await supabaseAdmin.auth.admin.generateLink({
        ...
        email: body.tenant_email,
        options: {
          redirectTo: `${Deno.env.get('FRONTEND_URL') || 'http://localhost:5173'}/complete-account?type=tenant`
        }
      });
      ...
      return new Response(JSON.stringify({ 
        ...
        message: 'Tenant account created successfully. Invitation email sent.'
      }), {
        status: 201,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
