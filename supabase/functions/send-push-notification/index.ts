...
serve(async (req) => {
  ...
  try {
    ...
    const { title, body, url, userIds, districtId, notificationType = 'general' } = await req.json();

    if (!title || !body) {
      throw new Error('Title and body are required');
    }

    console.log('Sending push notification:', { title, body, url, userIds, districtId, notificationType });
...
async function createPushRequest(endpoint: string, p256dh: string, auth: string, payload: string) {
