...
AS $$
  SELECT CASE 
    WHEN current_role = 'resident' AND requested_role IN ('community_leader', 'service_provider', 'facility_manager') THEN 'community_admin'::user_role
    WHEN current_role = 'resident' AND requested_role = 'security' THEN 'district_coordinator'::user_role
    WHEN current_role = 'community_admin' AND requested_role = 'district_coordinator' THEN 'state_admin'::user_role
    WHEN current_role = 'district_coordinator' AND requested_role = 'state_admin' THEN 'admin'::user_role
...
AS $$
  SELECT CASE 
    WHEN current_role = 'resident' AND requested_role = 'community_leader' THEN ARRAY['community_voting'::approval_requirement]
    WHEN current_role = 'resident' AND requested_role = 'service_provider' THEN ARRAY['business_verification'::approval_requirement]
    WHEN current_role = 'resident' AND requested_role = 'facility_manager' THEN ARRAY['interview_process'::approval_requirement]
    WHEN current_role = 'resident' AND requested_role = 'security' THEN ARRAY['background_check'::approval_requirement]
    WHEN current_role = 'community_admin' AND requested_role = 'district_coordinator' THEN ARRAY['performance_evaluation'::approval_requirement]
    WHEN current_role = 'district_coordinator' AND requested_role = 'state_admin' THEN ARRAY['multi_level_approval'::approval_requirement]
