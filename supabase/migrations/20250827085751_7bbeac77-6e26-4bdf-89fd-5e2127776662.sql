-- Rename district_features to community_features and update structure
ALTER TABLE public.district_features RENAME TO community_features;

-- Drop the district_id column and add community_id
ALTER TABLE public.community_features DROP COLUMN district_id;
ALTER TABLE public.community_features ADD COLUMN community_id UUID REFERENCES public.communities(id) ON DELETE CASCADE;
...
DROP POLICY "District coordinators can manage their district features" ON public.community_features;

-- Create new policies for community_features
...
-- Drop old function
DROP FUNCTION public.is_module_enabled_for_district(TEXT, UUID);