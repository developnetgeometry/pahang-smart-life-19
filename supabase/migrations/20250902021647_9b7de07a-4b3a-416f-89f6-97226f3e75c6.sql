...
-- Create functions for facility management
...
DECLARE
  ...
  v_facility_available BOOLEAN;
  v_operating_hours JSONB;
  v_day_name TEXT;
BEGIN
  -- Get facility buffer time and operating hours
  SELECT buffer_time_minutes, operating_hours, is_available, NOT maintenance_mode
  INTO v_buffer_minutes, v_operating_hours, v_facility_available
  FROM facilities 
  WHERE id = p_facility_id AND is_active = true;
  
  IF NOT FOUND OR NOT v_facility_available THEN
    RETURN FALSE;
  END IF;
  ...
  IF (v_operating_hours->v_day_name->>'closed')::BOOLEAN = true THEN
    RETURN FALSE;
  END IF;
  ...
  WHERE b.facility_id = p_facility_id
    ...
    AND (
      ...
      (p_start_time - INTERVAL '1 minute' * v_buffer_minutes) < b.end_time
      AND (p_end_time + INTERVAL '1 minute' * v_buffer_minutes) > b.start_time
    );
...
-- Function to get facility usage statistics
CREATE OR REPLACE FUNCTION public.get_facility_usage_stats(
  p_facility_id UUID,
...
BEGIN
  ...
  WITH booking_stats AS (
    SELECT 
      ...
      AVG(ul.user_rating) as avg_rating
    FROM bookings b
    LEFT JOIN facility_usage_logs ul ON ul.booking_id = b.id
    WHERE b.facility_id = p_facility_id
  ...
  operating_hours AS (
    SELECT 
      (p_end_date - p_start_date + 1) * 14 as total_operating_hours -- Assuming 14 hours per day average
  ),
  hourly_usage AS (
    SELECT 
  ...
  SELECT 
    bs.booking_count,
    bs.total_hours_booked,
    ROUND(bs.avg_rating, 2),
    CASE 
      WHEN oh.total_operating_hours > 0 
      THEN ROUND((bs.total_hours_booked / oh.total_operating_hours) * 100, 2)
      ELSE 0 
    END,
    (SELECT jsonb_agg(jsonb_build_object('hour', hour_of_day, 'bookings', booking_count)) 
     FROM hourly_usage)
  FROM booking_stats bs, operating_hours oh;
END;
$$;

-- Function to create automatic reminders for bookings
...
BEGIN
  ...
  v_facility_name := v_booking_record.facility_name;
  
  -- Create reminders
  INSERT INTO booking_reminders (booking_id, reminder_type, minutes_before, title, message)
  VALUES 
    -- 24 hour advance reminder
    (p_booking_id, 'advance', 1440, 
     'Booking Reminder - Tomorrow', 
     'You have a booking for ' || v_facility_name || ' tomorrow at ' || v_booking_record.start_time::TEXT),
    
    -- 1 hour before start
    (p_booking_id, 'start', 60, 
     'Booking Starting Soon', 
     'Your booking for ' || v_facility_name || ' starts in 1 hour'),
    
    -- 15 minutes before end
    (p_booking_id, 'end', 15, 
     'Booking Ending Soon', 
     'Your booking for ' || v_facility_name || ' ends in 15 minutes'),
    
    -- 15 minutes after end for cleanup reminder
    (p_booking_id, 'cleanup', -15, 
     'Cleanup Reminder', 
     'Please ensure ' || v_facility_name || ' is clean and ready for the next user');
END;
$$;

-- Trigger to automatically create reminders for new bookings
CREATE OR REPLACE FUNCTION public.handle_booking_reminders()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' AND OLD.status != 'confirmed' AND NEW.status = 'confirmed' THEN
    -- Delete any existing reminders and create new ones
    DELETE FROM booking_reminders WHERE booking_id = NEW.id;
    PERFORM create_booking_reminders(NEW.id);
  ELSIF TG_OP = 'UPDATE' AND NEW.status != 'confirmed' AND OLD.status = 'confirmed' THEN
    -- Delete reminders for cancelled/rejected bookings
    DELETE FROM booking_reminders WHERE booking_id = NEW.id AND sent_at IS NULL;
  END IF;
...
CREATE TRIGGER trigger_booking_reminders
  AFTER INSERT OR UPDATE ON bookings
  FOR EACH ROW
  EXECUTE FUNCTION handle_booking_reminders();
