-- Fix missing user roles (without created_at/updated_at columns)
...
AND NOT EXISTS (
  ...
  WHERE uc.user_id = p.id AND uc.community_id = 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa'::uuid AND uc.is_active = true
);
